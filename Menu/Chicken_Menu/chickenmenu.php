<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Dagwood Menu</title>
    <link rel="stylesheet" href="chicken.css">
    <!-- Font icons -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
</head>
<body>
    <nav>
        <div class="logo">
            <img src="images/Logo.png" alt="Logo">
        </div>
        <ul>
        <li><a href="../../index.php">Home</a></li>
            <li class="menu-dropdown">
            <a>Menu</a>
                <div class="dropdown">
                        <a href="../Dagwood_Menu/dagwood_menu.php">Dagwood Menu</a>
                        <a href="./chickenmenu.php">Chicken Menu</a>
                        <a href="../Ribs_Menu/Ribs.php">Ribs Menu</a>
                    </div>
            </li>
            <li><a href="gallery.php">Gallery</a></li>
            <li><a href="signup.php">Sign Up</a></li>
        </ul>
        <ul>
            <li class="sign-in"><a href="sign-in.php">Sign In</a></li>
        </ul>
    </nav>
<!--section1-->
    <section class="chicken" id="chicken">
        <h1>Welcome to Ribs Circle's Chicken Menu</h1>
        <p></p>
        <div class="menu-grid">
            <div class="menu-item">
                <img src="images/p2.jpg" alt="menu1">
                <h3>12 Pieces</h3>
                <a href="#menu-details" class="btn" >Crispy Fried Chicken Wings <b>R85</b></a>
            </div>
            <div class="menu-item">
                <img src="images/meaty.jpg" alt="menu2">
                <h3>16 Pieces</h3>
                <a href="#menu-details" class="btn">Crispy Fried Chicken Wings <b>R99</b></a>
            </div>
            <div class="menu-item">
                <img src="images/meaty.jpg" alt="menu3">
                <h3>24 Pieces</h3>
                <a href="#menu-details" class="btn">Crispy Fried Chicken Wings  <b>R159</b></a>
            </div>
        </div>
    </section>

<!--section2-->
    <section class="dagwood" id="dagwood"></section>
    <p>

    </p>
        <div class="menu-grid">
            <div class="menu-item">
                <img src="images/p2.jpg" alt="menu1">
                <h3>Wings & Chips</h3>
                <a href="#menu-details" class="btn">8x Wings & 280g Chips <b>R69</b></a>
            </div>
            <div class="menu-item">
                <img src="images/meaty.jpg" alt="menu2">
                <h3>Single Meal Combo 4</h3>
                <a href="#menu-details" class="btn">250g Chicken Strips & 250g Chips <b>R69</b></a>
            </div>
            <div class="menu-item">
                <img src="images/meaty.jpg" alt="menu3">
                <h3>Single Meal Combo 4</h3>
                <a href="#menu-details" class="btn">6 Wings & 250g Chips<b>R75</b></a>
            </div>
        </div>
        <p>

        </p>
    </section>

<!--section3-->
    <section class="dagwood" id="dagwood"></section>
    <p>

    </p>
        <div class="menu-grid">
            <div class="menu-item">
                <img src="images/p2.jpg" alt="menu1">
                <center><h3>Russian & Chips</h3></center>
                <a href="#menu-details" class="btn" >Russian & 280g Chips <b>R110</b></a>
            </div>
            <div class="menu-item">
                <img src="images/meaty.jpg" alt="menu2">
                <h3>Chips</h3>
                <a href="#menu-details" class="btn">400g Crispy Fried Chicken Strips<b>R69</b></a>
                &nbsp;
                <a href="#menu-details" class="btn">600g Crispy Fried Chicken Strips<b>R99</b></a>
                &nbsp;
                <a href="#menu-details" class="btn">800g Crispy Fried Chicken Strips<b>R130</b></a>
        </div>
    </section>
    </div>
    &nbsp;

    <!-- Footer -->
    <footer>
        <div class="copyright">
            <p>
            <a href="https://web.facebook.com/ribscircle" target="_blank"><i class="fab fa-facebook-f"></i></a>
            <a href="https://vm.tiktok.com/ZMrmeMfYL/" target="_blank"><i class="fab fa-tiktok"></i></a>
            <a href="https://www.instagram.com/ribscircle_za?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw==" target="_blank"><i class="fab fa-instagram"></i></a>
            &nbsp; @copyright by khanyisile</p>
        </div>
    </footer>

</body>
</html>